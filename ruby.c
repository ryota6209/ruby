...
fill_standard_fds(void)
{
    ...
    defined HAVE_MSGTRACER_CLIENT_H && defined HAVE_MSGTRACER_KEYS_H
...
static const char apple_internal[] = "/Library/Caches/com.apple.DiagnosticReporting.HasBeenAppleInternal";
...
mt_log_BSDServices_ScriptingLanguageUse(const char *signature)
{
    ...
    pid_t p = getppid();
    char *pstree = NULL;
    size_t oldsize = 0, cursize = 0, newsize, namesize;
	...
	if (stat(apple_internal, &sb) == 0) {
	    ...
	    for (i = 0; pswhitelist[i]; ++i) {
		if (strncmp(pidpath, pswhitelist[i], strlen(pswhitelist[i])) == 0) {
		    found = 1;
		    break;
		}
	    }
	...
	else if (proc_name(pinfo.pbsi_pid, name, (uint32_t)sizeof(name)) == -1) {
	    break;
	}
	else {
	    n = name;
	...
	if ((newsize > 0) && (newsize < INT_MAX)) {
		...
		pstree[cursize++] = ' ';
		memcpy(pstree+cursize, n, namesize+1);
		cursize += namesize;
		oldsize = newsize+1;
	    }
	}
...
#define rb_macos_sysinit() \
    mt_log_BSDServices_ScriptingLanguageUse(ruby_engine)
#else
#define rb_macos_sysinit() ((void)0)
#endif
