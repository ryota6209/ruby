...
static double
round_to_even(double x, double s)
{
    double f, d, xs = x * s;
...
rb_num_get_rounding_option(VALUE opts)
{
    ...
    if (!NIL_P(opts)) {
	if (!round_kwds[0]) {
	    round_kwds[0] = rb_intern_const("to");
	}
...
static SIGNED_VALUE
int_round_to_even(SIGNED_VALUE x, SIGNED_VALUE y)
{
...
static SIGNED_VALUE
int_round_to_nearest(SIGNED_VALUE x, SIGNED_VALUE y)
{
    return (x + y / 2) / y * y;
}
