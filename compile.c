...
static ibf_offset_t
ibf_dump_write(struct ibf_dump *dump, const void *buff, uint32_t size)
{
...
static void
ibf_dump_overwrite(struct ibf_dump *dump, void *buff, uint32_t size, long offset)
{
    VALUE str = dump->str;
...
static void *
ibf_load_alloc(const struct ibf_load *load, ibf_offset_t offset, long size)
{
...
ibf_table_index(struct st_table *table, st_data_t key)
{
	...
	index = rb_long2int(table->num_entries);
...
{
    const int size = RARRAY_LENINT(dump->iseq_list);
...
{
    const int size = rb_long2int(dump->id_table->num_entries);
...
ibf_dump_object_string(struct ibf_dump *dump, VALUE obj)
{
    ...
    uint32_t len = RSTRING_LENINT(obj);
...
ibf_load_object_string(const struct ibf_load *load, const struct ibf_object_header *header, ibf_offset_t offset)
{
    ...
    rb_enc_associate_index(str, (int)encindex);
...
{
    uint32_t len = BIGNUM_LENINT(obj);
...
ibf_dump_object_data(struct ibf_dump *dump, VALUE obj)
{
	...
	uint32_t len = rb_long2int(strlen(name) + 1);
	IBF_WV(type);
	IBF_WV(len);
...
static size_t
ibf_dumper_memsize(const void *ptr)
{
    struct ibf_dump *dump = (struct ibf_dump *)ptr;
...
static const rb_data_type_t ibf_dump_type = {
    "ibf_dumper",
    {ibf_dumper_mark, ibf_dumper_free, ibf_dumper_memsize,},
...
iseq_ibf_dump(const rb_iseq_t *iseq)
{
    ...
    dumper_obj = TypedData_Make_Struct(0, struct ibf_dump, &ibf_dump_type, dump);
    ibf_dump_setup(dump, dumper_obj);
    ...
    str = dump->str;
    ibf_dumper_free(dump);
    DATA_PTR(dumper_obj) = NULL;
