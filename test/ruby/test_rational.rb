...
class Rational_Test < Test::Unit::TestCase
  ...
  def test_trunc
    [[Rational(13, 5),  [ 2,  3,  2,  3,  3]], #  2.6
     [Rational(5, 2),   [ 2,  3,  2,  3,  2]], #  2.5
     [Rational(12, 5),  [ 2,  3,  2,  2,  2]], #  2.4
     [Rational(-12,5),  [-3, -2, -2, -2, -2]], # -2.4
     [Rational(-5, 2),  [-3, -2, -2, -3, -2]], # -2.5
     [Rational(-13, 5), [-3, -2, -2, -3, -3]], # -2.6
    ].each do |i, a|
      s = proc {i.inspect}
      ...
      assert_equal(a[4], i.round(to: :even), s)
    end
  end
